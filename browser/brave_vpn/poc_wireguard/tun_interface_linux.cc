...
LinuxTunInterface::DnsManager DetectDnsManager() {
  if (::system("which resolvectl > /dev/null 2>&1") == 0) {
    return LinuxTunInterface::DnsManager::kResolvectl;
  }
  if (::system("which resolvconf > /dev/null 2>&1") == 0) {
    return LinuxTunInterface::DnsManager::kResolvconf;
  }
  return LinuxTunInterface::DnsManager::kResolveDotConf;
}
...
bool LinuxTunInterface::Open() {
  ...
  int fd = HANDLE_EINTR(::open("/dev/net/tun", O_RDWR));
  if (fd < 0) {
